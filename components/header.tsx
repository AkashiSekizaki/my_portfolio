...
import { Menu, User, Code, Users, FlaskConical, GraduationCap } from "lucide-react"
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-14 items-center">
          <div className="mr-4 flex ml-4">
            <a className="mr-6 flex items-center space-x-2 cursor-pointer" onClick={() => handleNavClick("hero")}>
              <span className="font-bold text-xl">田中太郎</span>
            </a>
          </div>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`transition-colors hover:text-foreground/80 ${
                  currentPage === item.page ? "text-primary font-semibold" : "text-foreground/60"
                }`}
              >
                {item.name}
              </button>
          ...
          <div className="ml-auto mr-4">
            <SearchBar onResultClick={handleSearchResult} selectedTags={searchTags} onTagsChange={setSearchTags} />
          </div>
